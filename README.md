...
3. **Kontrol.** Alle forventede sider findes, `404.html` findes, ingen noindex, ingen tom style_push-pladsholder, dev-adressen findes ingen steder, intet spor af Visual Editor, hver fil siderne og stilarkene henviser til (src, href, srcset, `url()`) ligger i kopien, Cloudflares grænser (20.000 filer, 25 MiB pr. fil) overholdes. Én fejl stopper kørslen; live-sitet røres ikke. Formularer giver en advarsel (de virker først i trin 2), og det samme gør links til sider, der ikke er med i kopien. Eksterne hosts (fx Adobe Fonts) vises til orientering.
