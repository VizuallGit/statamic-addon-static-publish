...
4. **Upload.** `npx --yes wrangler@4 deploy` med en genereret `wrangler.jsonc` (assets-only Worker: `assets.directory`, `not_found_handling = 404-page`, `html_handling = auto-trailing-slash`, ingen `main`) i en midlertidig mappe, der slettes bagefter. Token og account ID gives som miljøvariabler til den ene proces.
